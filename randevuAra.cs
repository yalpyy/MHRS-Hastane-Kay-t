...
namespace HastaneKayit
{
    public partial class randevuAra : Form
    {
        ...
        void hastaneleriGoster()
        {

            frm1.uyelerBaglantisi.Open();
            try
            {
                hastanegoster.Items.Clear();
                OleDbCommand cmd = new OleDbCommand("Select * From hastaneler", frm1.uyelerBaglantisi);
                cmd.ExecuteNonQuery();
                OleDbDataReader rd = cmd.ExecuteReader();
                while (rd.Read())
                {
                    hastanegoster.Items.Add(rd["hastanead"].ToString());
                }
            }
                ...
                if (saatler.Text == sender.ToString().Substring(34))
                {
                    ...
                    if (saatler.BackColor!=Color.Red)
                    {
                        if (ayniSaatVarmi == false)
                        {
                            frm1.uyelerBaglantisi.Close();
                            DialogResult evetmi;
                            evetmi = MessageBox.Show(cmBoxRandevTarihi.Text + " Tarihinde " + sender.ToString().Substring(34) + " Saatinde " + hastanegoster.Text + "Hastanesinde" + cmBoxKlinik.Text + " Kliniğinde \n" + cmBoxDoktor.Text + " Tarafından Muayene İçin Randevu Alınıyor ", " YALPHS", MessageBoxButtons.YesNo);
                            if (evetmi.ToString() == "Yes")
                            {
                                try
                                {
                                    ...
                                    OleDbCommand cmd = new OleDbCommand("insert into Randevular (Tc,KlinikAdi,DoktorAdi,Tarih,Saat,hastane) values ('" + lblTC.Text + "','" + cmBoxKlinik.Text + "','" + cmBoxDoktor.Text + "','" + cmBoxRandevTarihi.Text + "','" + saatler.Text +"','"+ hastanegoster.Text + "')", frm1.uyelerBaglantisi);
                                    cmd.ExecuteNonQuery();
        ...
        private void hastanegoster_SelectedIndexChanged(object sender, EventArgs e)
        {
